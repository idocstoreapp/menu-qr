...
export default function MenuItemManagerV2() {
  ...
  const fetchData = async () => {
    try {
      const [itemsRes, categoriesRes] = await Promise.all([
        fetch('/api/menu-items-v2'),
        fetch('/api/categories-v2'),
      ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <button
          onClick={openNewForm}
          className="bg-gold-600 hover:bg-gold-500 text-arabic-dark px-4 py-2 rounded-lg transition flex items-center gap-2"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1">
          <input
            ...
            className="w-full bg-arabic-brown border border-gold-600 rounded-lg px-4 py-2 text-gold-100 focus:outline-none focus:border-gold-400"
          />
        </div>
        <select
          value={filterCategory === 'all' ? 'all' : filterCategory}
          onChange={(e) => setFilterCategory(e.target.value === 'all' ? 'all' : parseInt(e.target.value))}
          className="bg-arabic-brown border border-gold-600 rounded-lg px-4 py-2 text-gold-100 focus:outline-none focus:border-gold-400"
        >
      ...
      <div className="space-y-3">
        {filteredItems.map((item) => (
          <div
            key={item.id}
            className={`
              bg-arabic-dark border rounded-lg p-4 transition
      ...
      {isFormOpen && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-arabic-dark border-2 border-gold-600 rounded-xl p-6 w-full max-w-lg my-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="flex-1 bg-gold-600 hover:bg-gold-500 text-arabic-dark px-4 py-2 rounded-lg transition disabled:opacity-50"
                >
